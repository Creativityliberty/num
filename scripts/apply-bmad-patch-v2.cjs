...
function generatePersona(agentId) {
  const personas = {
    ...
    'prompt-engineer': {
      role: 'Prompt Engineering Expert + LLM Specialist',
      identity: 'Senior prompt engineer with 5+ years optimizing LLM interactions. Expert in prompt design patterns, advanced techniques (ToT, self-consistency, chaining), and evaluation methodologies.',
      communication_style: 'Technical and precise. Explains prompt optimization with practical examples. Advocates for measurable outcomes and reliability.',
      principles: '- Prompt quality directly impacts LLM output\n      - Measure and iterate on prompt performance\n      - Security and ethical design matter\n      - Token efficiency reduces costs\n      - Consistency across prompts improves reliability',
    },
  };
...
function generateWorkflows(agentId) {
  const workflows = {
    ...
    'prompt-engineer': [
      '  - trigger: optimize-prompt\n    description: "Optimize prompt for better LLM output"\n    workflow: "prompt-optimization-workflow"',
      '  - trigger: evaluate-prompt\n    description: "Evaluate prompt quality and effectiveness"\n    workflow: "prompt-evaluation-workflow"',
      '  - trigger: design-prompt\n    description: "Design new prompt from requirements"\n    workflow: "prompt-design-workflow"',
      '  - trigger: party-mode\n    description: "Collaborate with other agents"\n    workflow: "party-mode-workflow"',
    ],
  };
...
function generateCriticalActions(agentId) {
  const actions = {
    ...
    'prompt-engineer': [
      '  - "Always measure prompt effectiveness with metrics"',
      '  - "Validate prompt security and ethical implications"',
...
function applyBmadPatch(agentPath, agentId) {
  try {
    ...
    let isCustomModes = content.includes('customModes:');
    let isAgent = content.includes('agent:');
    let isSimpleYaml = !isCustomModes && !isAgent;

    let enriched = '';

    if (isCustomModes) {
      // Handle customModes structure
      enriched = 'customModes:\n';
      enriched += '- slug: ' + agentId + '\n';
      enriched += `  name: ${metadata.icon} ${agentId}\n`;
      enriched += `  persona:\n`;
      ...
      const otherContent = content.replace(/customModes:\n- slug:[\s\S]*?(?=\n\w|$)/, '');
      ...
      // Preserve other fields
      const otherContent = content.replace(/^agent:\n[\s\S]*?(?=\n\w|$)/, '');
      if (otherContent.trim()) {
        enriched += '\n' + otherContent;
      }
    } else {
      // Handle simple YAML structure (slug, name, etc.)
      enriched = `slug: ${agentId}\n`;
      enriched += `name: ${metadata.icon} ${agentId}\n`;
      enriched += `module: ${metadata.module}\n`;
      enriched += `persona:\n`;
      ...
      // Preserve other fields
      const otherContent = content.replace(/^(slug|name|module|persona|critical_actions|menu):[\s\S]*?(?=\n\w|$)/gm, '');
      if (otherContent.trim()) {
        enriched += '\n' + otherContent;
      }
    }
