...
export function executeGitOps(root: string, input: GitOpsInput): GitOpsResult {
  try {
    const cwd = root;
...
export function executeBatchOp(root: string, input: BatchOpInput): BatchOpResult {
  ...
  try {
    ...
    const items = (catalogData.items || catalogData.modes || []) as any[];
...
export function generateReport(root: string, input: ReportInput): ReportResult {
  try {
