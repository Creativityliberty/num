...
export type BatchApplyResult = {
  ...
  applied: Array<{ idx: number; ok: boolean; error?: string; written?: string[] }>;
  reportPath?: string;
};
...
export function batchSelectSuggestions(root: string, input: BatchApplyInput): BatchApplyItem[] {
  ...
  const uniq = new Map<string, (typeof suggestions)[0]>();
...
export function runBatchApply(root: string, input: BatchApplyInput): BatchApplyResult {
