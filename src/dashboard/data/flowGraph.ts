...
export function generateFlowGraph(mode: Record<string, unknown>): FlowGraph {
  const flow = mode.flow as Record<string, unknown> | undefined;
  if (!flow) {
    return {
  ...
  return {
    nodes,
    edges,
    title: (mode.name as string) || "Flow Graph",
  };
}

export function generateSVG(graph: FlowGraph): string {
  const width = 400;
...
export function calculateLayout(graph: FlowGraph): FlowGraph {
  const layoutGraph = JSON.parse(JSON.stringify(graph)) as FlowGraph;
