...
export function computeScoring(root: string, range: string, q?: string, tagContains?: string) {
  const dir = path.join(root, ".mcp", "runs");
  if (!fs.existsSync(dir)) return [] as ScoreRow[];

  const cutoff = parseRange(range || "7d");
  const minTs = cutoff ? nowMs() - cutoff : 0;
  const files = fs.readdirSync(dir).filter((f) => f.endsWith(".json"));

  const map = new Map<string, { runs: number; done: number; failed: number; cancelled: number; totalMs: number; countedMs: number; tags: Set<string> }>();
  ...
  for (const f of files) {
    ...
    if (!modeId || typeof modeId !== "string") continue;
    ...
    const tagsArr = (j.tags || j.modeTags || j.record?.tags || []) as any[];
    const tags = new Set<string>((Array.isArray(tagsArr) ? tagsArr : []).map((x) => String(x)));

    if (!map.has(modeId)) map.set(modeId, { runs: 0, done: 0, failed: 0, cancelled: 0, totalMs: 0, countedMs: 0, tags });
    const row = map.get(modeId)!;
    row.runs += 1;

    for (const t of tags) row.tags.add(t);

    if (status.includes("DONE")) row.done += 1;
    else if (status.includes("FAIL")) row.failed += 1;
    else if (status.includes("CANCEL")) row.cancelled += 1;
  ...
  for (const [id, v] of map.entries()) {
    ...
    if (qq && !id.toLowerCase().includes(qq)) continue;
    if (tt && !tagStr.includes(tt)) continue;
...
export function suggestAgents(scored: ScoreRow[], goal: string) {
  const g = (goal || "").toLowerCase();
  if (!g) return scored.slice(0, 5);
  ...
  const boosted = scored.map((x) => {
    ...
    for (const rule of boosts) {
      if (rule.t.some((w) => g.includes(w)) && x.id.toLowerCase().includes(rule.k)) b += 3;
    }
    return { ...x, _boost: b };
  });
