...
async function loadModernTemplate(): Promise<string> {
  try {
...
function modelHealthPage(): string {
  return `<!doctype html>
<html>
<head>
  <meta charset="utf-8"/>
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if ((u.pathname === "/api/modes/validate-simulate-all" || u.pathname === "/api/packops/validate-simulate-all") && req.method === "POST") {
      ...
      req.on("end", async () => {
        try {
          ...
          const filtered = all.filter((m: any) => String(m.id).startsWith(prefix));
          const ids = filtered.map((m: any) => String(m.id));
    ...
    if (u.pathname === "/api/smoke/start" && req.method === "POST") {
      ...
      req.on("end", async () => {
        try {
          ...
          const reg = new ModesRegistry({ modesPath });
          const all = await reg.list();
          const allModes = all.map((m: any) => ({ id: String(m.id), tags: m.tags ?? [] }));
            ...
            try {
              ...
              if (yamlContent) {
                ...
                for (const line of lines) {
                  ...
                  if (nameMatch && currentMode) currentMode.name = nameMatch[1].trim();
                }
                if (currentMode) customModes.push(currentMode);
              }
            } catch (e) {
    ...
    if (catalogModeMatch && req.method === "GET") {
      ...
      if (catalog && catalog.modes) {
        mode = catalog.modes.find((m: any) => m.id === modeId);
      }
    ...
    if (req.method === "GET" && u.pathname.startsWith("/api/flowgraph/")) {
      ...
      try {
        ...
        const graph = generateFlowGraph(mode as Record<string, unknown>);
