...
export class AgentAccessTester {
  ...
  async testAgentAccess(agentName: string): Promise<AgentAccessResult> {
  ...
  async testMultipleAgents(agentNames: string[]): Promise<AgentAccessResult[]> {
    const results: AgentAccessResult[] = [];
  ...
  async testClearInstructions(): Promise<PromptEngineeringTest> {
    const startTime = Date.now();
    const test: PromptEngineeringTest = {
...
3. Advanced reasoning capabilities with thinking mode`;
  ...
  async testFewShotExamples(): Promise<PromptEngineeringTest> {
    const startTime = Date.now();
    const test: PromptEngineeringTest = {
...
Now classify: "The Gemini API is amazing and powerful!"`,
  ...
  async testStructuredOutput(): Promise<PromptEngineeringTest> {
    const startTime = Date.now();
    const test: PromptEngineeringTest = {
      ...
      userPrompt: `Extract information about Gemini API and return as JSON:
{
  ...
  async testChainOfThought(): Promise<PromptEngineeringTest> {
    const startTime = Date.now();
    const test: PromptEngineeringTest = {
...
Answer: The total cost is $2.25`;
  ...
  async testRoleBasedPrompting(): Promise<PromptEngineeringTest> {
    const startTime = Date.now();
    const test: PromptEngineeringTest = {
...
5. **Scalability**
  ...
  async runPromptEngineeringTests(): Promise<PromptEngineeringTest[]> {
    return Promise.all([
...
// Main execution
export async function runAgentAccessAndPromptTests(apiKey: string): Promise<void> {
