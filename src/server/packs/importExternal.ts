...
export async function importExternalModes(input: ExternalImportInput) {
  const warnings: any[] = [];
  let agents: any[] = [];
  ...
  if (!input.dryRun) {
    ...
    if (input.writeManifestYaml !== false) {
      ...
      const chefs = normalized.modes.filter((m: any) => (m.tags ?? []).includes("chef"));
      const stubs = normalized.modes.filter((m: any) => (m.tags ?? []).includes("stub"));
      const aliases = normalized.modes.filter((m: any) => (m.tags ?? []).includes("alias"));
      const manifestData = {
        packId: input.packId,
        ...
        modes: normalized.modes.map((m: any) => ({
          id: m.id,
          name: m.name,
          tags: m.tags,
        ...
        warnings: warnings.map((w: any) => ({ kind: w.kind, message: w.message, ref: w.ref })),
      };
