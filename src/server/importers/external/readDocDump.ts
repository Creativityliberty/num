...
//   groups: [a,b]
//   customInstructions: ...
export function readDocDump(inputPath: string) {
  ...
  try {
    ...
    if (Array.isArray(j)) return { agents: j.map(mapLoose), warnings };
    if (Array.isArray(j?.customModes)) return { agents: j.customModes.map(mapLoose), warnings };
    if (j && typeof j === "object") return { agents: [mapLoose(j)], warnings };
  } catch {
    // fallthrough
  }
  ...
  const flush = () => {
    if (!cur.slug && !cur.name) return;
    if (instrLines.length) cur.customInstructions = instrLines.join("\n").trim();
  ...
  for (const line of lines) {
    ...
    if (m) {
      ...
      if (key === "slug" && (cur.slug || cur.name)) flush();
    ...
    if (collecting) instrLines.push(line);
  }
  flush();
...
function mapLoose(x: any): ExternalAgent {
...
function parseGroups(v: any): string[] | undefined {
