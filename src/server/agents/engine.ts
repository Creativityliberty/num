...
function buildPromptPack(role: AgentRole, run: any, _job: any) {
  const task = run.task ?? {};
  ...
  if (role === "planner") {
    return {
      system:
        "You are PlannerAgent. Produce STRICT JSON only. No prose. Output must match MultiPlanSchema: {jobs:[{jobId,role,goal,dependsOn?,status?}], acceptanceCriteria?, notes?}.",
      user:
  ...
  if (role === "implementer") {
    return {
      system:
        "You are ImplementerAgent. Produce STRICT JSON only. Output must match PatchCandidateSchema: {candidateId, patch, commands:[{cmd,args[]}], rationale?}. " +
  ...
  if (role === "reviewer") {
    ...
    return {
      system:
        "You are ReviewerAgent. Produce STRICT JSON only. Output must match ReviewReportSchema: {verdict: approve|changes|reject, summary?, findings:[{severity,type,message,files?}]}.",
      user:
  ...
  if (role === "arbiter") {
    ...
    return {
      system:
        "You are ArbiterAgent. Choose the best patch candidate. Output STRICT JSON only matching MergeDecisionSchema: {chosenCandidateId, rationale?, requiredFixes?[]}.",
      user:
  ...
  return {
    system:
      "You are SecurityAgent. Produce STRICT JSON only. Output must match SecurityReportSchema: {verdict: allow|block|manualConfirm, summary?, issues:[{type,message,severity}]}.",
    user:
      `Task goal:\n${task.goal}\n\nPatchCandidate:\n${JSON.stringify(cand ?? {}, null, 2)}\n\n` +
...
export class AgentsEngine {
  ...
  private modes?: ModesStore;

  constructor(opts: { policy: Policy; modes?: ModesStore }) {
  ...
  async plan(input: unknown) {
    ...
    if (parsed.dsl) {
      ...
      if (!this.modes) throw new Error("AgentsEngine: modeId provided but modes store not configured");
      const mode = await this.modes.get(parsed.modeId);
      if (!mode.flow) throw new Error(`Mode '${parsed.modeId}' has no flow`);
    ...
    const run = {
      ...
      fixIterations: 0,
      history: [{ ts: now, from: "INIT" as const, to: "NEEDS_PLAN" as const, note: historyNote }],
      agents: {
        jobs: jobs ?? [
          { jobId: "plan", role: "planner" as const, goal: "Create an execution plan", dependsOn: [], status: "pending" as const },
          { jobId: "implementA", role: "implementer" as const, goal: "Implement candidate A (patch + commands)", dependsOn: ["plan"], status: "pending" as const },
          { jobId: "implementB", role: "implementer" as const, goal: "Implement candidate B (patch + commands)", dependsOn: ["plan"], status: "pending" as const },
          { jobId: "merge", role: "arbiter" as const, goal: "Choose best candidate", dependsOn: ["implementA", "implementB"], status: "pending" as const },
          { jobId: "review", role: "reviewer" as const, goal: "Review chosen patch candidate", dependsOn: ["merge"], status: "pending" as const },
          { jobId: "security", role: "security" as const, goal: "Security/policy assessment", dependsOn: ["merge"], status: "pending" as const },
        ],
  ...
  async next(input: unknown): Promise<Next> {
    ...
    const done = new Set(jobs.filter((j: any) => j.status === "done").map((j: any) => j.jobId));
    const runnable = jobs.filter((j: any) => {
    ...
    const role = nextJob.role as AgentRole;

    // 12.6: if flow DSL present, use node.expectedSchema + templated prompt
    const flowSpec = run.agents?.artifacts?.flow;
    const node = flowSpec?.nodes?.find((n: any) => n.id === nextJob.jobId);

    let expectedSchema: string;
    let promptPack: { system: string; user: string };
    ...
    const runtimePolicy = resolveRuntimePolicy({
      ...
      defaults: (this.policy as any)?.runtimeDefaults,
    });
  ...
  async submit(input: unknown) {
    ...
    const run = (await tryLoadRun(this.policy, parsedIn.runId)) as any;
    if (!run) {
      return { runId: parsedIn.runId, jobId: parsedIn.jobId, ok: false, status: "failed", message: "Run not found" };
    }
    const jobs = run.agents?.jobs ?? [];
    const job = jobs.find((j: any) => j.jobId === parsedIn.jobId);
    if (!job) {
  ...
  async mergeDecision(input: unknown) {
    const parsedIn = AgentsMergeDecisionInputSchema.parse(input);
    const run = (await tryLoadRun(this.policy, parsedIn.runId)) as any;
    if (!run) return { runId: parsedIn.runId, ok: false, status: "failed", message: "Run not found" };
    ...
    const chosen = candidates.find((c: any) => c.candidateId === decision.chosenCandidateId);
    if (!chosen) {
    ...
    const jobs = run.agents?.jobs ?? [];
    const mergeJob = jobs.find((j: any) => j.jobId === "merge");
    if (mergeJob) {
