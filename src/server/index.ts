...
import {
    AgentsMergeDecisionInputSchema,
    AgentsMergeDecisionResultSchema,
    AgentsNextInputSchema,
    AgentsNextResultSchema,
    AgentsPlanInputSchema,
    AgentsSubmitInputSchema,
    AgentsSubmitResultSchema,
    BundlePrCreateInputSchema,
    ExecRunInputSchema,
    ...
    ModesPlanInputSchema,
    ModesPlanPromptInputSchema,
    ModesRenderInputSchema,
    ModesReviewPromptInputSchema,
    ModesRunPromptInputSchema,
    ModesSuggestInputSchema,
    OrchestrateCancelInputSchema,
    OrchestrateContinueInputSchema,
    OrchestrateRunInputSchema,
    OrchestrateStatusInputSchema,
    PipelineApplyAndVerifyInputSchema,
...
export async function startMcpServer(opts: { modesPaths: string[]; bus: EventBus; policy: Policy }): Promise<void> {
  ...
  server.tool(
    "modes.list",
    "List available modes (normalized).",
    {
  ...
  server.tool(
    "modes.get",
    "Get a mode by id (normalized, with source metadata).",
    {
    ...
    async (raw) => {
      ...
      if (!mode) {
        ...
        return {
          content: [{ type: "text" as const, text: JSON.stringify({ error: "MODE_NOT_FOUND", id: input.id }) }],
        };
      }
      ...
      return {
        content: [{ type: "text" as const, text: JSON.stringify(mode, null, 2) }],
      };
    }
  );
  ...
  server.tool(
    ...
    async (raw) => {
      ...
      const sessionId = ensureSessionId((raw as Record<string, unknown>).sessionId as string | undefined);
      const t0 = performance.now();
  ...
  server.tool(
    "modes.render",
    "Render a universal prompt-pack for a given mode + task envelope.",
    {
      id: z.string().min(1),
      task: z.unknown(),
    ...
    async (raw) => {
      ...
      return {
        content: [{ type: "text" as const, text: JSON.stringify({ sessionId, pack }, null, 2) }],
      };
    }
  );
  ...
  server.tool(
    "modes.plan",
    "Return a deterministic, server-side plan (no LLM). If modeId is omitted, auto-selects via suggest().",
    {
      sessionId: z.string().optional(),
    ...
    async (raw) => {
      ...
      let selectedMeta: { id: string; name: string; confidence?: number } | undefined;
      ...
      if (!selectedMode) {
        ...
        if (best) {
          ...
          if (selectedMode) selectedMeta = { id: selectedMode.id, name: selectedMode.name, confidence: best.confidence };
        }
      } else {
  ...
  server.tool(
    "modes.planPrompt",
    "Return a planning-only prompt pack (LLM-assisted plan generated by the IDE's chosen model). If modeId omitted, auto-selects via suggest().",
    {
      sessionId: z.string().optional(),
      modeId: z.string().optional(),
      task: z.unknown(),
    ...
    async (raw) => {
      ...
      // Select mode (explicit > auto)
      let selectedMode = input.modeId ? registry.get(input.modeId) : undefined;
      let selectedConfidence: number | undefined;
      ...
      if (!selectedMode) {
        ...
        return {
          content: [{ type: "text" as const, text: JSON.stringify({ error: "MODE_NOT_FOUND", id: input.modeId ?? null }) }],
        };
      }
  ...
  server.tool(
    "modes.runPrompt",
    "Return an implementation prompt-pack that forces the IDE model to output a structured bundle (patch + commands + PR).",
    {
      sessionId: z.string().optional(),
      modeId: z.string().optional(),
      task: z.unknown(),
  ...
  server.tool(
    "modes.reviewPrompt",
    "Return a review prompt-pack that forces the IDE model to output structured findings (severity, issues, approval).",
    {
      sessionId: z.string().optional(),
    ...
    async (raw) => {
      ...
      if (!mode) {
        ...
        return {
          content: [{ type: "text" as const, text: JSON.stringify({ error: "MODE_NOT_FOUND", id: input.modeId }) }],
        };
      }
  ...
  server.tool(
    ...
    "Validate and optionally apply a unified diff to the workspace. Use dryRun=true for validation only.",
    {
      sessionId: z.string(),
      diff: z.string().min(1),
  ...
  server.tool(
    "workspace.backup",
    "Backup files (usually before applyPatch) into .mcp/backups/<runId>/",
    {
      sessionId: z.string(),
    ...
    async (raw) => {
      const input = raw as { sessionId: string; runId: string; files: string[] };
      const t0 = performance.now();
  ...
  server.tool(
    "workspace.rollback",
    "Rollback workspace files from .mcp/backups/<runId>/",
    {
      sessionId: z.string(),
      runId: z.string().min(1),
    },
    async (raw) => {
      const input = raw as { sessionId: string; runId: string };
      const t0 = performance.now();
  ...
  server.tool(
    "exec.run",
    "Execute an allowlisted command (cmd + args, no shell) in a sandboxed environment with timeout and output limits.",
    {
      sessionId: z.string(),
  ...
  server.tool(
    "git.status",
    "Get git status (porcelain) for the workspaceRoot (policy-controlled).",
    {
      sessionId: z.string().optional(),
    },
    async (raw) => {
      ...
      try {
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ sessionId, ...s }, null, 2) }] };
      } catch (e: unknown) {
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ error: "GIT_STATUS_FAILED", message: msg }) }] };
      }
    }
  );
  ...
  server.tool(
    "git.diff",
    "Get git diff for workspaceRoot (policy-controlled).",
    {
      sessionId: z.string().optional(),
    ...
    async (raw) => {
      ...
      try {
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ sessionId, diff: d }, null, 2) }] };
      } catch (e: unknown) {
        const msg = e instanceof Error ? e.message : String(e);
        opts.bus.emitEvent("tool.failed", {
          sessionId,
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ error: "GIT_DIFF_FAILED", message: msg }) }] };
      }
    }
  );
  ...
  server.tool(
    "git.log",
    "Get git log summary for workspaceRoot (policy-controlled).",
    {
      sessionId: z.string().optional(),
    ...
    async (raw) => {
      ...
      try {
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ sessionId, log: out }, null, 2) }] };
      } catch (e: unknown) {
        const msg = e instanceof Error ? e.message : String(e);
        opts.bus.emitEvent("tool.failed", {
          sessionId,
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ error: "GIT_LOG_FAILED", message: msg }) }] };
      }
    }
  );
  ...
  server.tool(
    "git.branch.create",
    "Create a git branch (policy-controlled, prefix allowlist).",
    {
      sessionId: z.string().optional(),
    ...
    async (raw) => {
      ...
      try {
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ sessionId, ok: true, name: input.name }, null, 2) }] };
      } catch (e: unknown) {
        const msg = e instanceof Error ? e.message : String(e);
        opts.bus.emitEvent("tool.failed", {
          sessionId,
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ error: "GIT_BRANCH_CREATE_FAILED", message: msg }) }] };
      }
    }
  );
  ...
  server.tool(
    "git.commit",
    "Commit changes (policy-controlled). Optionally add -A. Supports dryRun.",
    {
      sessionId: z.string().optional(),
    ...
    async (raw) => {
      ...
      try {
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ sessionId, commitSha: sha, dryRun: input.dryRun }, null, 2) }] };
      } catch (e: unknown) {
        const msg = e instanceof Error ? e.message : String(e);
        opts.bus.emitEvent("tool.failed", {
          sessionId,
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ error: "GIT_COMMIT_FAILED", message: msg }) }] };
      }
    }
  );

  server.tool(
    "pipeline.applyAndVerify",
    "Apply a patch and run verification commands (policy-controlled). Stops on first failure.",
    {
      sessionId: z.string().optional(),
      diff: z.string().min(1),
    ...
    async (raw) => {
      ...
      try {
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ sessionId, ...res }, null, 2) }] };
      } catch (e: unknown) {
        const msg = e instanceof Error ? e.message : String(e);
        opts.bus.emitEvent("tool.failed", {
          sessionId,
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ error: "PIPELINE_FAILED", message: msg }) }] };
      }
    }
  );
  ...
  server.tool(
    "bundle.pr.create",
    "Create an exportable PR bundle for the session. Optionally writes .mcp/bundles/<sessionId>.json",
    {
      sessionId: z.string().optional(),
      modeId: z.string().optional(),
    ...
    async (raw) => {
      ...
      try {
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ sessionId, writtenPath, bundle }, null, 2) }] };
      } catch (e: unknown) {
        const msg = e instanceof Error ? e.message : String(e);
        opts.bus.emitEvent("tool.failed", {
          sessionId,
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ error: "BUNDLE_FAILED", message: msg }) }] };
      }
    }
  );
  ...
  server.tool(
    "orchestrate.run",
    "Start an orchestration run (LLM-in-the-loop via orchestrate.continue).",
    {
      sessionId: z.string().optional(),
      task: z.object({
        goal: z.string(),
    ...
    async (raw) => {
      ...
      try {
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ error: "ORCH_RUN_FAILED", message: msg }) }] };
      }
    }
  );
  ...
  server.tool(
    ...
    "Continue an orchestration run by submitting LLM outputs (plan/run/review).",
    {
    ...
    async (raw) => {
      ...
      try {
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ error: "ORCH_CONTINUE_FAILED", message: msg }) }] };
      }
    }
  );
  ...
  server.tool(
    ...
    async (raw) => {
      ...
      try {
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ error: "ORCH_STATUS_FAILED", message: msg }) }] };
      }
    }
  );
  ...
  server.tool(
    ...
    async (raw) => {
      ...
      try {
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ ok: true, runId: input.runId, state }, null, 2) }] };
      } catch (e: unknown) {
        const msg = e instanceof Error ? e.message : String(e);
        opts.bus.emitEvent("tool.failed", {
          sessionId,
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ error: "ORCH_CANCEL_FAILED", message: msg }) }] };
      }
    }
  );
  ...
  server.tool(
    "agents.plan",
    "Create a serial multi-agent run (planner→implementer→reviewer→security). Returns runId.",
    {
      task: z.object({
    ...
    async (raw) => {
      ...
      try {
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ error: "AGENTS_PLAN_FAILED", message: msg }) }] };
      }
    }
  );
  ...
  server.tool(
    "agents.next",
    "Get next pending job + promptPack + expected schema.",
    {
    ...
    async (raw) => {
      ...
      try {
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ error: "AGENTS_NEXT_FAILED", message: msg }) }] };
      }
    }
  );
  ...
  server.tool(
    "agents.submit",
    "Submit job payload (validated) and advance state.",
    {
    ...
    async (raw) => {
      ...
      try {
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ error: "AGENTS_SUBMIT_FAILED", message: msg }) }] };
      }
    }
  );
  ...
  server.tool(
    "agents.mergeDecision",
    "Choose best candidate among multiple implementers and materialize chosen patchCandidate/runOutput.",
    {
      runId: z.string().min(1).describe("Run ID"),
      decision: z.object({
    ...
    async (raw) => {
      ...
      try {
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ error: "AGENTS_MERGE_FAILED", message: msg }) }] };
      }
    }
  );
  ...
  server.tool(
    "runs.telemetry.append",
    "Append model telemetry event to a run (chosen model, attempts, usage). Best-effort, non-blocking.",
    {
      runId: z.string().min(1).describe("Run ID"),
      event: z.object({
    ...
    async (raw) => {
      ...
      try {
        if (opts.policy.allowWrite === false) {
          return { content: [{ type: "text" as const, text: JSON.stringify({ ok: false, error: "POLICY_BLOCK", message: "allowWrite=false" }) }] };
        }
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ ok: false, error: "TELEMETRY_APPEND_FAILED", message: msg }) }] };
      }
    }
  );
  ...
  server.tool(
    "pack.import.external",
    "Import external agents (YAML or doc dump) into Num modes + pack.json.",
    {
    ...
    async (raw) => {
      ...
      try {
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify(ExternalImportResultSchema.parse(result)) }] };
      } catch (e: unknown) {
        const msg = e instanceof Error ? e.message : String(e);
        opts.bus.emitEvent("tool.failed", {
          sessionId,
        ...
        return { content: [{ type: "text" as const, text: JSON.stringify({ ok: false, error: "IMPORT_FAILED", message: msg }) }] };
      }
    }
  );
