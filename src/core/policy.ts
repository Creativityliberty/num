 ...
 */
export function isExecAllowed(policy: Policy, cmd: string, args: string[]): { ok: boolean; reason?: string } {
  ...
  if (allowedExecs.length > 0) {
    ...
    if (patterns && patterns.length > 0) {
      ...
      if (!match) return { ok: false, reason: `Args not allowed for ${cmd}: ${args.join(" ")}` };
    }
    return { ok: true };
  }
  ...
  if (legacy.length > 0) {
    ...
    if (!legacy.includes(full)) return { ok: false, reason: `Command not in allowedCommands: ${full}` };
    return { ok: true };
  }
...
function sameArgs(pattern: string[], args: string[]): boolean {
 ...
 */
export function requiresConfirmation(policy: Policy, what: ConfirmationTarget): boolean {
 ...
 */
export function isWritePathAllowed(policy: Policy, filePath: string): { ok: boolean; reason?: string } {
