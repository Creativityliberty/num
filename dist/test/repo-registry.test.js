...
describe("12.9 repo structured → modes", () => {
    it("loads modes from repo packs", async () => {
        ...
        await fs.writeFile(path.join(packDir, "pack.json"), JSON.stringify({ id: "roo-core", name: "Roo Core", version: "1.0.0", tags: ["core"] }));
        await fs.writeFile(path.join(packDir, "modes", "fix.json"), JSON.stringify({
            id: "fix",
            name: "Fix Mode",
            tags: ["bugfix"],
            flow: {
    ...
    it("get() returns repo mode detail", async () => {
        ...
        await fs.writeFile(path.join(packDir, "modes", "agent.json"), JSON.stringify({
            id: "agent",
            name: "Test Agent",
            flow: {
                version: "1",
    ...
    it("skips disabled packs", async () => {
        ...
        await fs.writeFile(path.join(packDir, "modes", "a.json"), JSON.stringify({
            id: "a",
            flow: {
                version: "1",
                nodes: [{ id: "n", role: "planner", goal: "g", expectedSchema: "multiPlan", prompt: { system: "s", user: "u" } }],
    ...
    it("aggregates FILE, CATALOG, and REPO sources", async () => {
        ...
        await fs.mkdir(modesPath, { recursive: true });
        // FILE mode
        await fs.writeFile(path.join(modesPath, "file-mode.json"), JSON.stringify({
            id: "file-mode",
            flow: {
                version: "1",
                nodes: [{ id: "n", role: "planner", goal: "g", expectedSchema: "multiPlan", prompt: { system: "s", user: "u" } }],
        ...
        await fs.writeFile(catPath, JSON.stringify({
            version: "1",
            namespace: "cat",
            agents: [
                {
        ...
        await fs.writeFile(path.join(packDir, "modes", "repo-mode.json"), JSON.stringify({
            id: "repo-mode",
            flow: {
                version: "1",
                nodes: [{ id: "n", role: "planner", goal: "g", expectedSchema: "multiPlan", prompt: { system: "s", user: "u" } }],
...
//# sourceMappingURL=repo-registry.test.js.map