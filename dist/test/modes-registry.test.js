...
describe("12.7 Modes Registry", () => {
    ...
    it("registry.list: returns modes with validation status", async () => {
        ...
        // Valid mode
        await fs.writeFile(path.join(modesPath, "valid.json"), JSON.stringify({
            id: "valid",
            name: "Valid Mode",
            tags: ["test"],
            flow: {
        ...
        // Mode without flow
        await fs.writeFile(path.join(modesPath, "noflow.json"), JSON.stringify({ id: "noflow", name: "No Flow" }));
        // Mode with cycle
        await fs.writeFile(path.join(modesPath, "cycle.json"), JSON.stringify({
            id: "cycle",
            flow: {
                version: "1",
                nodes: [
        ...
        await fs.writeFile(path.join(modesPath, "badschema.json"), JSON.stringify({
            id: "badschema",
            flow: {
                version: "1",
    ...
    it("registry.get: returns mode detail", async () => {
        ...
        const modesPath = path.join(tmp, "modes");
        await fs.mkdir(modesPath, { recursive: true });
        await fs.writeFile(path.join(modesPath, "test.json"), JSON.stringify({
            id: "test",
            name: "Test Mode",
            flow: {
                version: "1",
    ...
    it("registry.simulate: returns ticks and blockedBy", async () => {
        ...
        const modesPath = path.join(tmp, "modes");
        await fs.mkdir(modesPath, { recursive: true });
        await fs.writeFile(path.join(modesPath, "sim.json"), JSON.stringify({
            id: "sim",
            flow: {
                version: "1",
                nodes: [
    ...
    it("registry.validate: returns validation result", async () => {
        ...
        const modesPath = path.join(tmp, "modes");
        await fs.mkdir(modesPath, { recursive: true });
        await fs.writeFile(path.join(modesPath, "val.json"), JSON.stringify({
            id: "val",
            flow: {
                version: "1",
...
//# sourceMappingURL=modes-registry.test.js.map