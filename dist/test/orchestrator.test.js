...
describe("orchestrator (step 8)", () => {
    it("fails gracefully when no modes are available", async () => {
        ...
        const bus = createEventBus({ maxEvents: 50, log: logger() });
        const start = await orchestrateStart({ policy, bus, registry }, {
...
`;
        ...
        expect(start.nextStep.stepId).toBe("plan");
...
`;
        ...
        expect(start.nextStep.stepId).toBe("plan");
        // Continue with plan
        const c1 = await orchestrateContinue({ policy, bus, registry }, {
            runId: start.runId,
            stepId: "plan",
        ...
        expect(c1.nextStep.stepId).toBe("run");
        // Continue with run output
        const c2 = await orchestrateContinue({ policy, bus, registry }, {
            runId: start.runId,
            stepId: "run",
        ...
        expect(c2.nextStep.stepId).toBe("review");
        // Continue with review (approved)
        const c3 = await orchestrateContinue({ policy, bus, registry }, {
            runId: start.runId,
            stepId: "review",
