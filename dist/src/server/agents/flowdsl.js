import { FlowSpecSchema } from "../../core/schemas.js";
...
export function normalizeFlowSpec(input) {
    const spec = FlowSpecSchema.parse(input);
    // Ensure unique node ids
    const ids = new Set();
    for (const n of spec.nodes) {
        if (ids.has(n.id))
            throw new Error(`FlowSpec: duplicate node id: ${n.id}`);
        ids.add(n.id);
    }
    // Validate edges refer to known nodes
    for (const e of spec.edges ?? []) {
        if (!ids.has(e.from))
            throw new Error(`FlowSpec: edge.from unknown: ${e.from}`);
        if (!ids.has(e.to))
            throw new Error(`FlowSpec: edge.to unknown: ${e.to}`);
    }
    return spec;
}
export function buildJobsFromFlow(spec) {
    const deps = {};
    for (const n of spec.nodes)
        deps[n.id] = [];
    for (const e of spec.edges ?? []) {
