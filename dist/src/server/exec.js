...
import { isExecAllowed } from "../core/policy.js";
...
function sanitizeCmd(cmd) {
    if (/\s/.test(cmd))
        throw new Error("Invalid cmd: whitespace not allowed");
    if (cmd.includes("/") || cmd.includes("\\"))
        throw new Error("Invalid cmd: path separators not allowed (use executable name)");
    return cmd;
}
function ensureInsideWorkspace(workspaceRoot, cwd) {
...
function isBlockedToken(cmd, args) {
...
export async function runCommand(opts) {
    ...
    if (blocked.blocked)
        throw new Error(`Command blocked token: ${blocked.token}`);
...
export async function runCommandWithPolicy(policy, opts) {
    const startTime = performance.now();
    const cmd = sanitizeCmd(opts.cmd.trim());
    const args = (opts.args ?? []).map(String);
    const commandStr = [cmd, ...args].join(" ").trim();
...
export function isCommandAllowed(command, customAllowlist) {
