...
function defaultVerification(hints) {
    const v = ["Review changes for scope + safety."];
...
export function buildDeterministicPlan(opts) {
    ...
    if (errs)
        acceptanceCriteria.push("The previously failing signal no longer reproduces.");
    ...
    if (!task.context?.openFiles?.length)
        assumptions.push("Relevant files will be identified during implementation.");
    risks.push("Hidden coupling may require adjusting related modules beyond the initially identified scope.");
    if (includesAny(goal, ["refactor", "cleanup"]))
        risks.push("Refactors can introduce subtle behavior changes—prefer small steps.");
    return {
