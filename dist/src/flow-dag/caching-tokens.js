...
export class ContextCachingManager {
    caches = new Map();
    defaultTTL = '3600s'; // 1 hour default
    // Create a new cache
    createCache(config) {
        const cache = {
    ...
    updateCacheExpireTime(name, expireTime) {
        const cache = this.caches.get(name);
        if (cache) {
...
export class TokenCounter {
    ...
    meetsMinimumCache(model, tokenCount) {
        const limits = this.getModelLimits(model);
        if (!limits)
            return false;
    ...
    calculateCostSavings(cachedTokens, requestCount) {
...
export class CachingTokensHandler {
    ...
    getImplicitCachingInfo(model, tokenCount) {
        return {
    ...
    getCachingStrategy(model, tokenCount, requestCount) {
        ...
        if (this.tokenCounter.meetsMinimumCache(model, tokenCount) &&
            requestCount >= 2) {
...
export function createCachingTokensHandler() {
