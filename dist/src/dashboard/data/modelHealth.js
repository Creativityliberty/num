...
export function loadTelemetryEvents(root) {
...
export function computeModelHealthFromEvents(events, range) {
    ...
    for (const e of filtered) {
        ...
        row.calls++;
        if (e.ts > row.lastSeen)
            row.lastSeen = e.ts;
        if (e.status === 429)
            row.rate429++;
        if (e.status && e.status >= 500 && e.status < 600)
            row.err5xx++;
        if (e.error?.toLowerCase().includes("timeout"))
            row.timeouts++;
        if (typeof e.latencyMs === "number")
            row.latencies.push(e.latencyMs);
    }
    const rows = [];
    let totalCalls = 0;
    let total429 = 0;
    ...
    for (const r of byModel.values()) {
        ...
        const score = Math.max(0, Math.min(100, Math.round(100 - (r429 * 30) - (r5xx * 40) - (rTo * 20) - latencyPenalty)));
        rows.push({
            provider: r.provider,
            model: r.model,
            calls: r.calls,
...
export function suggestFallbacks(report, minCalls = 20) {
    const suggestions = [];
    ...
    for (const [provider, models] of byProvider) {
        ...
        for (let i = 1; i < sorted.length; i++) {
            ...
            if (better.healthScore <= worse.healthScore)
                continue;
            const delta = better.healthScore - worse.healthScore;
            if (delta < 5)
                continue; // Not significant
            let confidence = "low";
            if (delta >= 20 && better.calls >= 50)
                confidence = "high";
            else if (delta >= 10)
                confidence = "medium";
            suggestions.push({
