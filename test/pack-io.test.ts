...
describe("13.2 pack export/import", () => {
  it("exports a pack to a bundle JSON", async () => {
    ...
    await fs.writeFile(
      path.join(repo, "roo", "pack.json"),
      JSON.stringify({ id: "roo", version: "1.0.0", name: "Roo Pack" }, null, 2)
    );
    await fs.writeFile(
      path.join(repo, "roo", "modes", "fix.json"),
      JSON.stringify({ id: "fix", name: "Fix Mode" }, null, 2)
    );
    ...
    expect(bundle.files.some((f: any) => f.path === "pack.json")).toBe(true);
    expect(bundle.files.some((f: any) => f.path === "modes/fix.json")).toBe(true);
  });
  ...
  it("imports a pack bundle into repo", async () => {
    ...
    await fs.writeFile(
      path.join(repo, "test-pack", "pack.json"),
      JSON.stringify({ id: "test-pack", version: "2.0.0" }, null, 2)
    );
    await fs.writeFile(
      path.join(repo, "test-pack", "modes", "a.json"),
