...
import { hasPermission, ROLE_PERMISSIONS, type User } from "../src/core/auth.schema.js";
...
describe("14.1 auth & JWT", () => {
  ...
  it("rejects expired token", () => {
    const token = createToken(
      { sub: "user-1", email: "test@example.com", role: "developer", tenantId: "acme" },
      { expirySeconds: -1 }
    );
    const payload = verifyToken(token);
    expect(payload).toBeNull();
  ...
  it("prevents duplicate email", async () => {
    ...
    await expect(
      store.createUser({ id: "u2", email: "dup@test.com", password: "p", role: "viewer", tenantId: "t", enabled: true })
    ).rejects.toThrow("USER_EXISTS");
  });

  it("hasPermission checks role permissions", () => {
    const admin: User = { id: "a", email: "a@t.com", passwordHash: "x", role: "admin", tenantId: "t", enabled: true };
    const dev: User = { id: "d", email: "d@t.com", passwordHash: "x", role: "developer", tenantId: "t", enabled: true };
    const viewer: User = { id: "v", email: "v@t.com", passwordHash: "x", role: "viewer", tenantId: "t", enabled: true };
