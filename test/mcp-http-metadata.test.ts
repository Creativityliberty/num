...
describe("18.2/18.3 tool metadata", () => {
  ...
  beforeAll(async () => {
    ...
    mcpServer.tool(
      ...
      async (input) => ({ sum: (input as { a: number; b: number }).a + (input as { a: number; b: number }).b }),
      {
  ...
  it("lists tools with inputSchema", async () => {
    ...
    const j = (await r.json()) as { ok: boolean; tools: Array<{ name: string; inputSchema: unknown }> };
  ...
  it("returns tool detail with inputSchema and description", async () => {
    ...
    expect(r.status).toBe(200);
    const j = (await r.json()) as { ok: boolean; tool: { name: string; description: string; inputSchema: { properties: { message: { type: string } } } } };
    expect(j.ok).toBe(true);
  ...
  it("returns outputSchema and examples for tools that have them", async () => {
    ...
    expect(r.status).toBe(200);
    const j = (await r.json()) as { ok: boolean; tool: { outputSchema: { properties: { sum: { type: string } } }; examples: Array<{ title: string }> } };
    expect(j.ok).toBe(true);
...
describe("18.4 OpenAPI", () => {
  ...
  it("serves OpenAPI spec", async () => {
    ...
    expect(r.status).toBe(200);
    const j = (await r.json()) as { openapi: string; info: { title: string }; paths: Record<string, unknown> };
